  ...
  try{
      if (typeof (wtappsms) == 'object') {
          ...
          if(event_type == 'User initiated'){
            ...
            // var values = wtappsms.eventContent.message[each]
            ...
            if(values.message[each]) {
              ...
              if(each == 'contacts'){
                console.log(JSON.stringify(values.message[each]))
                values.message[each]['contacts'] = JSON.stringify(values.message[each])
              }
              acc.push(values.message[each])
            }
            /* if(values) {
              mapers[each]?values['attachmentType'] = each : {}
              maps2[each]?wtappsms.eventContent.message.contextmid?values['contextmid'] = wtappsms.eventContent.message.contextmid : {}:{}
              acc.push(values)
            } */
            return acc
            },[]).filter(each => {
